...
int main(int argc, char const *argv[]) {
  ...
  intermadiate_cc->students->push_back(move(st1));
  intermadiate_cc->students->push_back(move(st2));
  intermadiate_cc->students->push_back(move(st3));
  ...
  for (int i = 0; i < 3; i++){
			//TODO local to insert a shared point.
      auto s = move(intermadiate_cc->students->at(i));
      s->sitInClass();
      s->printAllInfos();
  }
